...
<html lang="en">
    ...
    $search=$conn->query("SELECT COUNT(*) FROM artworks WHERE title LIKE '%$key%' OR artist LIKE '%$key%' OR description LIKE '%$key%'");
    ...
    $page=1;
    $artist='';
    $result = $conn->query("SELECT * FROM artworks WHERE title LIKE '%$key%' OR artist LIKE '%$key%' OR description LIKE '%$key%' LIMIT " . (12 * $page - 12) . ",12");
    ?>
    <form>
        ...
        function ajax(t=1,order=-1) {
            ...
            xml.onreadystatechange=function () {
                if (xml.readyState === 4 && xml.status === 200) {
                    document.getElementById("searchResult").innerHTML = xml.responseText;
                    setCookie("order",order);
                }
            };

            xml.open("GET","searchDeal.php?page="+t+"&order="+order,false);
            xml.send();
        }
    </script>
